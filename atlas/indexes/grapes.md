# Grapes

Governed grape profiles, including honest stubs, are listed here.

<!-- BEGIN GENERATED CARTA INDEX -->
## Grape profiles

### Human Reference surfaces

- [Altesse](../grapes/altesse.md) — `node` / `stub`
- [Arinto dos Açores](../grapes/arinto-dos-acores.md) — `node` / `stub`
- [Arriloba](../grapes/arriloba.md) — `node` / `stub`
- [Bouchalès](../grapes/bouchales.md) — `node` / `stub`
- [Cabernet Franc](../grapes/cabernet-franc.md) — `node` / `stub`
- [Cabernet Sauvignon](../grapes/cabernet-sauvignon.md) — `node` / `stub`
- [Carignan](../grapes/carignan.md) — `node` / `stub`
- [Chardonnay](../grapes/chardonnay.md) — `node` / `stub`
- [Chenin Blanc](../grapes/chenin-blanc.md) — `node` / `stub`
- [Clairette](../grapes/clairette.md) — `node` / `stub`
- [Courbu](../grapes/courbu.md) — `baseline` / `published`
- [Folgasão](../grapes/folgasao.md) — `node` / `stub`
- [Gamay noir à jus blanc](../grapes/gamay-noir-a-jus-blanc.md) — `node` / `stub`
- [Glera](../grapes/glera.md) — `node` / `stub`
- [Gouais blanc](../grapes/gouais-blanc.md) — `node` / `stub`
- [Graciano](../grapes/graciano.md) — `node` / `stub`
- [Gros Manseng](../grapes/gros-manseng.md) — `baseline` / `published`
- [Jacquère](../grapes/jacquere.md) — `node` / `stub`
- [Loureiro](../grapes/loureiro.md) — `node` / `stub`
- [Mencía](../grapes/mencia.md) — `node` / `stub`
- [Menu Pineau / Orbois](../grapes/menu-pineau.md) — `node` / `stub`
- [Mondeuse](../grapes/mondeuse.md) — `node` / `stub`
- [Mourvèdre](../grapes/mourvedre.md) — `node` / `stub`
- [Palomino Fino](../grapes/palomino-fino.md) — `node` / `stub`
- [Petit Courbu](../grapes/petit-courbu.md) — `baseline` / `published`
- [Petit Manseng](../grapes/petit-manseng.md) — `baseline` / `published`
- [Raffiat de Moncade](../grapes/raffiat-de-moncade.md) — `node` / `stub`
- [Riesling](../grapes/riesling.md) — `node` / `stub`
- [Sangiovese](../grapes/sangiovese.md) — `node` / `stub`
- [Savagnin](../grapes/savagnin.md) — `node` / `stub`
- [Syrah](../grapes/syrah.md) — `node` / `stub`
- [Tempranillo](../grapes/tempranillo.md) — `node` / `stub`
- [Viura](../grapes/viura.md) — `node` / `stub`

### Explicit machine-only dispositions

- **Bourboulenc** — `node` / `machine_only`
- **Caracol** — `node` / `machine_only`
- **Carmenère** — `node` / `machine_only`
- **Garnacha Tinta** — `node` / `machine_only`
- **Grenache Blanc** — `node` / `machine_only`
- **Merlot** — `node` / `machine_only`
- **Muscat Blanc** — `node` / `machine_only`
- **Petit Meslier** — `node` / `machine_only`
- **Petite Sirah** — `node` / `machine_only`
- **Pinot Gris** — `node` / `machine_only`
- **Pinot Noir** — `node` / `machine_only`
- **Sauvignon** — `node` / `machine_only`
- **Ugni Blanc** — `node` / `machine_only`

<!-- END GENERATED CARTA INDEX -->

This index is generated from `data/reference-profiles/`. Edit the governed profile record rather than this file.
