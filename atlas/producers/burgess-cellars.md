# Burgess Cellars / Promiscua

> **Navigation node:** this honest stub keeps a meaningful CARTA subject discoverable without presenting it as a finished baseline reference.

The machine graph and generated relationships below provide the current orientation. A generous subject-specific enrichment pass is required before baseline promotion.

<!-- BEGIN GENERATED CARTA NAVIGATION -->
## Explore CARTA

This section is generated from governed profile dispositions, editorial anchors, and supported graph relationships. It is not a hand-maintained second knowledge graph.

- [Cabernet Sauvignon](../grapes/cabernet-sauvignon.md) — grape; navigation node
- [Napa Valley AVA](../countries/united-states/appellations/napa-valley-ava.md) — appellation; navigation node
- [California](../countries/united-states/regions/california.md) — region; navigation node
- [United States](../countries/united-states/README.md) — country; navigation node
<!-- END GENERATED CARTA NAVIGATION -->

<!-- BEGIN GENERATED CARTA PROVENANCE -->
## Record & provenance

This section is generated from CARTA machine authority. Edit the governed records, then run `python scripts/validate_data.py --write-human-reference`.

- **Profile:** `profile:burgess-cellars`
- **Maturity / publication:** `node` / `stub`
- **Primary entity:** `producer:burgess-cellars`

**Component entities**

- `producer:burgess-cellars`
- `person:tom-burgess`
- `person:meghan-zobeck`
- `person:reid-griggs`
- `wine:burgess-promiscua`
- `institution:lawrence-wine-estates`
- `institution:demeine-estates`

**Representative anchors**

- `place:california`
- `appellation:napa-valley-ava`
- `grape:cabernet-sauvignon`
- `wine:burgess-promiscua`

<details>
<summary>Machine claims and sources</summary>

### Material claims

| Claim | Layer / observed | Status | Confidence | Sources |
|---|---|---|---|---|
| `claim:burgess-demeine-distribution-09` | `reference / 2026-08-19` | `supported` | `medium` | `source:burgess-demeine` |
| `claim:burgess-identity-history-09` | `reference / —` | `supported` | `high` | `source:burgess-history-trade`, `source:burgess-demeine` |
| `claim:burgess-promiscua-2021-09` | `reference / —` | `supported` | `high` | `source:burgess-promiscua-2021` |
| `claim:burgess-promiscua-identity-09` | `reference / —` | `supported` | `high` | `source:burgess-promiscua-2021`, `source:burgess-promiscua-current` |
| `claim:burgess-promiscua-napa-09` | `reference / 2026-08-19` | `supported` | `high` | `source:burgess-promiscua-current` |
| `claim:burgess-winemaker-succession-09` | `reference / 2026-08-19` | `supported` | `medium` | `source:burgess-promiscua-2021`, `source:burgess-demeine` |

### Sources

- `source:burgess-demeine` — Burgess
- `source:burgess-history-trade` — Burgess Cellars history and 2020 transition
- `source:burgess-promiscua-2021` — Promiscua Cabernet Sauvignon 2021 technical note
- `source:burgess-promiscua-current` — 2022 Promiscua Cabernet Sauvignon

</details>

### Open questions

- Reconcile Promiscua vineyard sources by vintage, especially Clos Abeille in 2021
- Do not promote organic/regenerative farming language into certification without certificate-level authority
<!-- END GENERATED CARTA PROVENANCE -->
