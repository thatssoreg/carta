# Domaine des Terres Dorées

**Charnay, southern Beaujolais, France**

Domaine des Terres Dorées supplies a southern Beaujolais counterweight to a cru-only regional story. The current public record places the domaine in Charnay and describes activity around the Terres Dorées as well as Fleurie, Moulin-à-Vent, Brouilly and Morgon.

## The 2024 transition

In September 2024, Domaines Roger Zannier announced that Jean-Paul Brun had entrusted operation and development of the domaine to the group from the 2024 harvest. The same announcement says Brun would accompany a five-year transition focused on making the wines.

CARTA records bounded `WORKED_WITH` relationships. It does **not** infer an asset sale, a completed retirement or an ownership transfer beyond the sources’ wording.

## Quantities held back

Rhône Tourisme currently gives a 57-hectare headline but describes 37 hectares around the domaine plus 18 hectares in the crus. Because 37 + 18 does not equal 57, CARTA records the mismatch as an open question and publishes no manufactured total.

## Producer base

The current public address, **565 route d’Alix, Charnay**, is geocoded as the production/business base. It is not a polygon for southern holdings or cru parcels.

Cellar-method comparisons involving Jean-Paul Brun or the new operating team must remain dated across this transition.

<!-- BEGIN GENERATED CARTA NAVIGATION -->
## Explore CARTA

This section is generated from governed profile dispositions, editorial anchors, and supported graph relationships. It is not a hand-maintained second knowledge graph.

- [Beaujolais](../countries/france/regions/beaujolais.md) — region; baseline reference
- [Morgon AOP](../countries/france/appellations/morgon.md) — appellation; baseline reference
- [Chardonnay](../grapes/chardonnay.md) — grape; navigation node
- [France](../countries/france/README.md) — country; baseline reference
- [Gamay noir à jus blanc](../grapes/gamay-noir-a-jus-blanc.md) — grape; baseline reference
- [Jean-Michel Stéphan](jean-michel-stephan.md) — producer; navigation node
<!-- END GENERATED CARTA NAVIGATION -->

<!-- BEGIN GENERATED CARTA PROVENANCE -->
## Record & provenance

This section is generated from CARTA machine authority. Edit the governed records, then run `python scripts/validate_data.py --write-human-reference`.

- **Profile:** `profile:domaine-des-terres-dorees`
- **Maturity / publication:** `baseline` / `published`
- **Primary entity:** `producer:domaine-des-terres-dorees`

**Component entities**

- `producer:domaine-des-terres-dorees`
- `person:jean-paul-brun`
- `institution:domaines-roger-zannier`

**Representative anchors**

- `place:charnay-rhone`
- `place:beaujolais`
- `appellation:fleurie`
- `appellation:moulin-a-vent`
- `appellation:brouilly`
- `appellation:morgon`
- `grape:gamay-noir-a-jus-blanc`
- `grape:chardonnay`

<details>
<summary>Machine claims and sources</summary>

### Material claims

| Claim | Layer / observed | Status | Confidence | Sources |
|---|---|---|---|---|
| `claim:stephan-beaujolais-route-17` | `reference / —` | `supported` | `high` | `source:idealwine-stephan` |
| `claim:terres-dorees-base-15` | `reference / 2026-08-30` | `supported` | `high` | `source:terres-dorees-rhone-tourisme`, `source:api-adresse-gouv-fr` |
| `claim:terres-dorees-current-identity-15` | `reference / 2026-08-30` | `supported` | `high` | `source:terres-dorees-rhone-tourisme` |
| `claim:terres-dorees-southern-chardonnay-16` | `reference / 2026-08-30` | `supported` | `medium` | `source:terres-dorees-rhone-tourisme` |
| `claim:terres-dorees-transition-2024-15` | `reference / 2026-08-30` | `supported` | `high` | `source:zannier-terres-dorees-transition`, `source:terres-dorees-rhone-tourisme` |

### Sources

- `source:api-adresse-gouv-fr` — API Adresse
- `source:idealwine-stephan` — Jean-Michel Stephan: outstanding, natural Côte-Rôtie
- `source:terres-dorees-rhone-tourisme` — Domaine des Terres Dorées à Charnay
- `source:zannier-terres-dorees-transition` — Les Domaines Roger Zannier investissent le Beaujolais

</details>

### Open questions

- Reconcile the current source’s 57-hectare headline with its 37 + 18 hectare subtotal before publishing a total
- Date any cellar-protocol claims across the 2024 operational transition
- Do not infer an asset sale or Jean-Paul Brun’s retirement from operation/development language
- `claim:terres-dorees-current-identity-15` — Do not publish a total-hectare measure until the current source’s 57-hectare headline and 37 + 18 hectare subtotal are reconciled.
<!-- END GENERATED CARTA PROVENANCE -->
