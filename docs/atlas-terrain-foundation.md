...
The first governed environmental dataset is the Copernicus DEM GLO-30 tile set. It was established over the western Pyrenees in the Béarn / Jurançon world, then applied through the same recipe and source policy to Beaujolais and two bounded Loire proofs: Savennières / Layon and Sancerre / Pouilly-sur-Loire. Its manifest is [`data/geography/datasets/copernicus-dem-glo30-2022-05-09.json`](../data/geography/datasets/copernicus-dem-glo30-2022-05-09.json); the establishing run is recorded in [`audits/run-08-atlas-terrain-foundation.md`](../audits/run-08-atlas-terrain-foundation.md), the Beaujolais application in [`audits/run-09-beaujolais-atlas-world.md`](../audits/run-09-beaujolais-atlas-world.md), and the Loire application in [`audits/run-17-loire-valley-world.md`](../audits/run-17-loire-valley-world.md).

The manifest pins ten 1° source tiles in four explicit, discontinuous groups: six for Béarn / Jurançon, two for Beaujolais, one for Savennières / Layon, and one for Sancerre / Pouilly-sur-Loire. Its outer bounding box is only an envelope around those groups, not France-wide coverage. The public descriptor therefore carries a `terrains` array, and the application lazy-loads each proof independently only when the viewport intersects it at a supported zoom. The intervening country receives no terrain.

One honest caveat belongs in the reader's mind and in this document. Copernicus DEM is a digital **surface** model: it records the top of whatever stood on the ground when TanDEM-X flew, including forest canopy and buildings. In wooded country a few metres of apparent height can be trees, not slope. That is acceptable for reading landform at regional scale, and it is one of the reasons this contract requires `measurement.scale_limitations` on every environmental dataset.
