...
The first governed environmental dataset is the Copernicus DEM GLO-30 tile set. It was established over the western Pyrenees in the Béarn / Jurançon world and is now applied, through the same recipe and source policy, to a second bounded proof in Beaujolais. Its manifest is [`data/geography/datasets/copernicus-dem-glo30-2022-05-09.json`](../data/geography/datasets/copernicus-dem-glo30-2022-05-09.json); the establishing run is recorded in [`audits/run-08-atlas-terrain-foundation.md`](../audits/run-08-atlas-terrain-foundation.md), and the Beaujolais application in [`audits/run-09-beaujolais-atlas-world.md`](../audits/run-09-beaujolais-atlas-world.md).

The manifest pins eight 1° source tiles in two explicit, discontinuous groups: six for Béarn / Jurançon and two for Beaujolais. Its outer bounding box is only an envelope around those groups, not France-wide coverage. The public descriptor therefore carries a `terrains` array, and the application lazy-loads each proof independently only when the viewport intersects it at a supported zoom. The intervening country receives no terrain.
